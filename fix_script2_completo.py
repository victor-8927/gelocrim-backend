...
# Encontra início e fim do segundo script
start = content.find('<script>\nvar _clientesCache')
end   = content.find('</script>', start) + len('</script>')

print(f'Substituindo script de {content[:start].count(chr(10))+1} até {content[:end].count(chr(10))+1}')

novo_script = r"""<script>
var _clientesCache = {};
var _todosClientes = [];
var _allOrders = [];
var _csvDados  = [];
...
# Substitui o segundo script
content = content[:start] + novo_script + content[end:]
