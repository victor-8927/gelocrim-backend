...
router = APIRouter(prefix="/api/v1/reports", tags=["Relatorios"])
...
def dashboard(_=Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    routes_today = db.execute(text("""
        ...
        FROM routes WHERE route_date = :today AND status NOT IN ('cancelled','draft')
    """), {"today": today}).fetchone()
    v = db.execute(text("SELECT COUNT(*) AS count FROM vehicles WHERE status = 'active'")).fetchone()
    d = db.execute(text("SELECT COUNT(*) AS count FROM drivers  WHERE status = 'active'")).fetchone()
    t = dict(totals._mapping)
    r = dict(routes_today._mapping)
    return {
        "date": today,
        "orders": {"pending": int(t["orders_pending"]), "routed": int(t["orders_routed"]),
                   "delivered": int(t["orders_delivered"]), "failed": int(t["orders_failed"])},
        "routes_today": {"count": int(r["count"]), "total_km": round(float(r["total_km"]),1),
                         "total_stops": int(r["total_stops"])},
        "fleet": {"vehicles_active": int(dict(v._mapping)["count"]),
                  "drivers_active":  int(dict(d._mapping)["count"])},
    }


@router.get("/summary")
def summary(start: date = Query(default_factory=date.today),
    ...
    row = db.execute(text("""
        ...
        WHERE r.route_date BETWEEN :s AND :e AND r.status NOT IN ('cancelled','draft')
    """), {"s": str(start), "e": str(end)}).fetchone()
    agg = dict(row._mapping)
    orow = db.execute(text("""
        SELECT COUNT(CASE WHEN status='routed'  THEN 1 END) AS routed,
               COUNT(CASE WHEN status='pending' THEN 1 END) AS pending
        FROM orders WHERE date(created_at) BETWEEN :s AND :e
    """), {"s": str(start), "e": str(end)}).fetchone()
    ...
    return {"period_start": str(start), "period_end": str(end),
            "total_routes": int(agg["total_routes"]), "total_stops_planned": plan,
            "total_stops_completed": comp, "total_stops_failed": int(agg["total_stops_failed"]),
...
@router.get("/vehicles")
def vehicles_perf(start: date = Query(default_factory=date.today),
                  end:   date = Query(default_factory=date.today),
                  _=Depends(get_current_user), db: Session = Depends(get_db)):
    rows = db.execute(text("""
        SELECT v.id AS vehicle_id, v.plate,
               ...
               COALESCE(SUM(r.total_distance_km),0) AS total_distance_km
        FROM vehicles v JOIN routes r ON r.vehicle_id=v.id LEFT JOIN stops s ON s.route_id=r.id
        WHERE r.route_date BETWEEN :s AND :e AND r.status NOT IN ('cancelled','draft')
        GROUP BY v.id, v.plate ORDER BY stops_completed DESC
    """), {"s": str(start), "e": str(end)}).fetchall()
    result = []
    for r in rows:
        d = dict(r._mapping)
        total = int(d["stops_completed"]) + int(d["stops_failed"])
        result.append({**d, "delivery_rate_pct": _pct(int(d["stops_completed"]),total),
                       "total_distance_km": round(float(d["total_distance_km"]),1)})
    return result


@router.get("/orders/failures")
def failures(start: date = Query(default_factory=date.today),
             end:   date = Query(default_factory=date.today),
             _=Depends(get_current_user), db: Session = Depends(get_db)):
    rows = db.execute(text("""
        SELECT COALESCE(s.failure_reason,'nao informado') AS failure_reason, COUNT(*) AS count
        FROM stops s JOIN routes r ON r.id=s.route_id
        WHERE s.status='failed' AND r.route_date BETWEEN :s AND :e
        GROUP BY failure_reason ORDER BY count DESC
    """), {"s": str(start), "e": str(end)}).fetchall()
    data  = [dict(r._mapping) for r in rows]
    total = sum(d["count"] for d in data)
    return [{"failure_reason": d["failure_reason"], "count": int(d["count"]),
             "pct_of_failures": _pct(int(d["count"]),total)} for d in data]
