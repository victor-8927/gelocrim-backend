...
def get_order(oid: str, _=Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    itens = db.execute(text("""
        ...
        WHERE codparc = :codparc
          AND negotiation_date = (
              SELECT MAX(negotiation_date)
              FROM order_items
              WHERE codparc = :codparc
          )
...
def create_order(order: dict = Body(...), db: Session = Depends(get_db)):
    ...
    try:
        ...
        existente = db.execute(
            text("SELECT id FROM orders WHERE external_id = :ext"),
            {"ext": ext_id}
        ).fetchone()
        ...
        db.execute(text("""
            INSERT INTO orders (
                id, external_id, source, recipient_id,
                lat, lng, weight_kg, volume_m3,
                tw_start, tw_end, notes, status, priority,
                delivery_date, created_at, updated_at,
                total_value, order_type,
                recipient_name, address, region, codparc
            ) VALUES (
                :id, :ext, :src, :rid,
                :lat, :lng, :kg, :m3,
                :tws, :twe, :notes, :status, :priority,
                :ddate, :ts, :ts,
                :tv, :ot,
                :rname, :addr, :region, :codparc
            )
        """), {
            "id":     oid,
            "ext":    ext_id,
...
# ─────────────────────────────────────────────
# DELETE /  — apaga pedidos pendentes/roteados
# ─────────────────────────────────────────────
@router.delete("", status_code=200)
def delete_pending_orders(_=Depends(get_current_user), db: Session = Depends(get_db)):
    result = db.execute(
        text("DELETE FROM orders WHERE status IN ('pending','routed')")
    )
    db.commit()
...
# ─────────────────────────────────────────────
# PATCH /{oid}/status  — atualiza status
# ─────────────────────────────────────────────
...
def update_order_status(
    ...
    db.execute(
        text("UPDATE orders SET status = :s, updated_at = :ts WHERE id = :id"),
