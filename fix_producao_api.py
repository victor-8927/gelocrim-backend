...
cur.execute('''CREATE TABLE IF NOT EXISTS itens_producao (
    id          TEXT PRIMARY KEY,
    nome        TEXT NOT NULL,
    peso        REAL DEFAULT 0,
    comprimento REAL DEFAULT 0,
    largura     REAL DEFAULT 0,
    altura      REAL DEFAULT 0,
    un_pallet   INTEGER DEFAULT 0,
    top         TEXT DEFAULT "1000",
    observacao  TEXT,
    created_at  TEXT,
    updated_at  TEXT
)''')
...
if cur.fetchone()[0] == 0:
    cur.execute("INSERT INTO pallets VALUES (?,?,?,?,?,?,?,?,?,?)",
        (str(uuid4()), 'Pallet Padrão', 1.20, 1.00, 0.15, 0.18, 1000, 'Pallet padrão de madeira', ts, ts))
    print('Pallet padrão inserido!')

# Itens de gelo padrão
cur.execute("SELECT COUNT(*) FROM itens_producao")
if cur.fetchone()[0] == 0:
    itens = [
        ('Gelo 5kg',  5,  180),
        ('Gelo 10kg', 10, 110),
    ...
    for nome, peso, un in itens:
        cur.execute("INSERT INTO itens_producao VALUES (?,?,?,?,?,?,?,?,?,?,?)",
            (str(uuid4()), nome, peso, 0.30, 0.20, 0.15, un, '1000', None, ts, ts))
    print('Itens padrão inseridos!')

conn.commit()
conn.close()
print('Banco de produção criado!\n')

# ── 2. Cria router de produção ─────────────────────────────────────
...
with open(r'C:\fleet-cloud\app\routers\producao.py', 'w', encoding='utf-8') as f:
    f.write(api_content)
print('producao.py criado!')
...
if 'producao' not in main:
    old_import = 'from app.routers import'
    # Encontra o bloco de imports de routers
    idx = main.find('from app.routers.vehicles import router as vehicles_router')
    if idx != -1:
        main = main[:idx] + 'from app.routers.producao import router as producao_router\n' + main[idx:]

    # Registra o router
    old_include = 'app.include_router(vehicles_router)'
    if old_include in main:
        main = main.replace(old_include, old_include + '\napp.include_router(producao_router)')

    with open(main_path, 'w', encoding='utf-8') as f:
        f.write(main)
    print('main.py atualizado com producao router!')
else:
    print('producao já registrado no main.py')
