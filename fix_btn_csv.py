...
# Adiciona botão após o botão de sincronizar
old_sync_btn = '''          <button class="btn btn-secondary" onclick="sincronizarSankhya()" style="border-color:#64B4FF;color:#64B4FF">
            🔄'''

new_sync_btn = '''          <button class="btn btn-primary" onclick="abrirImportacaoCSV()">📥 Importar CSV</button>
...
if old_sync_btn in content:
    ...
    print('Botão Importar CSV adicionado!')
else:
    # Tenta padrão simples
    content = content.replace(
        "onclick=\"loadOrders()\">↺ Atualizar</button>",
        "onclick=\"loadOrders()\">↺ Atualizar</button>\n          <button class=\"btn btn-primary\" onclick=\"abrirImportacaoCSV()\">📥 Importar CSV</button>"
    )
    print('Botão adicionado (alternativo)!')

# Verifica se o modal existe
if 'modal-importacao-csv' not in content:
    print('Modal não encontrado, adicionando...')
    # Adiciona modal antes da tela de roteirização
    old_rot = '    <div class="page" id="page-roteirizacao">'
    modal_html = '''    <!-- MODAL IMPORTAÇÃO CSV -->
      ...
      <div style="background:#0f2040;border:1px solid #1e3a5c;border-radius:16px;width:700px;max-height:90vh;overflow-y:auto">
        <div style="padding:18px 24px;border-bottom:1px solid #1e3a5c;display:flex;justify-content:space-between;align-items:center;position:sticky;top:0;background:#0f2040;border-radius:16px 16px 0 0">
          <div>
            ...
            <div style="font-size:11px;color:#90afd4;margin-top:2px">Campos usados: NUNOTA, NOMEPARC, ENDERECO, PESO, VOLUME, CODTIPOPER, VLRNOTA</div>
          </div>
        ...
        <div style="padding:20px 24px">
          <div style="background:#0a1628;border:1px solid #1e3a5c;border-radius:8px;padding:14px;margin-bottom:20px">
            <div style="font-size:11px;font-weight:700;color:#64B4FF;margin-bottom:8px">📋 FORMATO DO CSV</div>
            <div style="font-family:monospace;font-size:11px;color:#90afd4;line-height:1.8">
              ...
              Demais colunas: <span style="color:#90afd4">ignoradas automaticamente</span>
            </div>
          </div>
          <div style="border:2px dashed #1e3a5c;border-radius:10px;padding:30px;text-align:center;margin-bottom:20px;cursor:pointer" onclick="document.getElementById('csv-file-input').click()">
    ...
    if old_rot in content:
        content = content.replace(old_rot, modal_html + old_rot)
