...
export default function EntregaScreen({ navigation, route }) {
  ...
  async function confirmarEntrega() {
    ...
    try {
      ...
      if (!base64 && foto.uri) {
        base64 = await FileSystem.readAsStringAsync(foto.uri, { encoding: FileSystem.EncodingType.Base64 });
      }
      ...
      const res = await fetch(`${API_URL}/routes/${routeId}/stops/${stop.stop_id}`, {
        ...
        body: JSON.stringify({
          ...
          foto_base64: base64 ? `data:image/jpeg;base64,${base64}` : null,
        }),
      });
      if (res.ok) {
        Alert.alert('✅ Entrega Confirmada!', `${stop.recipient_name} registrado com sucesso!`, [
          { text:'OK', onPress:() => navigation.goBack() }
        ]);
      } else {
        const err = await res.json();
        Alert.alert('Erro', err.detail || 'Falha ao confirmar entrega.');
      }
...
with open(r'C:\gelocrim-motorista\screens\EntregaScreen.js', 'w', encoding='utf-8') as f:
    f.write(conteudo)
print('EntregaScreen.js atualizado com envio de foto!')
